...
public class BottomLeft extends LJPattern {
    ...
    public boolean checkPatternBodyOne(Matrix matrix, int row, int column){
        if( checkCorner(matrix, row, column, Position.TOPLEFT, true) && checkCorner(matrix, row, column + 2, Position.TOPLEFT, true) && checkCorner(matrix, row, column + 3, Position.TOPRIGHT, false) ){
    ...
    public boolean checkPatternBodyTwo(Matrix matrix, int row, int column){
        if( checkCorner(matrix, row, column, Position.TOPLEFT, true) && checkCorner(matrix, row, column -2 , Position.TOPLEFT, true) && checkCorner(matrix, row, column + 1, Position.TOPRIGHT, false)){
            matrix.setUsedForQuest(row,column);
            matrix.setUsedForQuest(row,column-2);
